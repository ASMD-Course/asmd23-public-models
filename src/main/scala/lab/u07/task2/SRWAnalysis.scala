...
object SRWAnalysis extends App:
  ...
  val targetMarking =  MSet(P1, P6, P5) // MSet(P1, P7)
  val simulationRuns = 100
  val rnd = new Random()

  val prefVariations = Seq(100_000.0, 200_000.0, 300_000.0)
  val writeSpeedVariations = Seq(0.1, 0.2, 0.4)

  val analyzer = SPNAnalyzer[Place](
    Trn(MSet(P1), _ => 1.0, MSet(P2), MSet()),
    Trn(MSet(P2), _ => 100_000, MSet(P3), MSet())
      .rateValues(prefVariations *),
    Trn(MSet(P2), _ => 100_000, MSet(P4), MSet())
      .rateValues(prefVariations *),
    ...
    Trn(MSet(P7), _ => 0.2, MSet(P1, P5), MSet())
      .rateValues(writeSpeedVariations *)
  )
